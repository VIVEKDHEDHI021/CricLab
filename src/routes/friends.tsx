      ...
      <div className="space-y-2">
        {list.map((f) => (
          <Card key={f.id} className="p-3 rounded-2xl flex items-center justify-between">
            {f.profile?.id ? (
              <Link to="/players/$id" params={{ id: f.profile.id }} className="hover:opacity-80 transition-opacity flex-1 text-left">
                <div>
                  <div className="font-medium text-foreground hover:underline">{f.profile?.name ?? "—"}</div>
            ...
            <Button size="icon" variant="outline" onClick={() => del(f.id)} className="ml-2"><Trash2 className="h-4 w-4" /></Button>
          </Card>
        ))}
