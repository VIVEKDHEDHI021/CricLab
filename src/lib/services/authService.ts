  ...
  async login(
    ...
    try {
      ...
      if (response.data && response.data.success) {
        ...
        // Persist the Sanctum token to localStorage so all subsequent API calls are authenticated
        localStorage.setItem('criclab_token', token);

        // Run full sync database migration from API before logging the user in
        try {
          if (onSyncProgress) {
